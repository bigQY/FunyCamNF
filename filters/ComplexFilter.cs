...
{
    class ComplexFilter : BaseInPlacePartialFilter
    {
        ...
        private unsafe byte[] pixelProcess(byte* srcData, int length, int w, int h)
        {
            ...
            //第四种滤镜（反色）
            for(int i = centerX; i < w; i++)
            {
                for(int j = centerY; j < h; j++)
                {
                    int tempB = i * 3 + j * w * 3;
                    int tempG = i * 3 + j * w * 3 + 1;
                    int tempR = i * 3 + j * w * 3 + 2;
                    tempData[tempB] = (byte)(255 -srcData[tempB]);
                    tempData[tempG] = (byte)(255 - srcData[tempG]);
                    tempData[tempR] = (byte)(255 - srcData[tempR]);
